...
small_index = sample.int(nrow(df), 5000)
...
plot_profile(df$loss[train_index],  df$loss[train_index], df[["f81"]][train_index], bucket_count = 25, error_band = 'normal')
...
```{r submit, echo=FALSE}
  ...
  file = file.path(working_folder, "Playground/Aug2021/submit_v8.lgb.csv")
...
my_params = data.table(expand.grid(
                       ...
                       min_data = seq(500, 800, by = 25), #default = 20
