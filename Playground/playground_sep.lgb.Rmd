...
lgb_vars = c(n_vars, 'claim_res1') %!in_set% c('id') #best performance
...
```{r submit, echo=FALSE}
  ...
  file = file.path(working_folder, "Playground/data/submit_v01.lgb.csv")
...
```{r save, echo=FALSE}
  ...
  rbind(df_sol, df[, .(id, claim = claim_prob, it = 1)])
  
  #fwrite(df[test_index, .(id, target=target_lgb)], file = file, row.names = FALSE)
  fwrite(rbind(df_sol, df[, .(id, claim = claim_prob, it = 1)]), file = file, row.names = FALSE)
