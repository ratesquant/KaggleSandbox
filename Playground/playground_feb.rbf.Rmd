...
models_boost =  rbf_boost.create(dfs, target, 2, max_nodes = 1024*2, n_runs = 30, max_it = 20, growth_rate = 1.5, kernel_fun = rbf_mquad_kernel, dist_fun = 'L1', adaptive =TRUE )
...
run_cases = expand.grid(nodes = c(1024), runs = c(20, 30), dist_fun = c('L1'), rbf_kernel = c('rbf_mquad_kernel','rbf_gauss_kernel'), kernel_scale = c(1, 2), growth_rate = c(1.5, 2.0), 
                        adaptive = c(FALSE, TRUE), var_set = c('pvars', 'qvars'), stringsAsFactors = FALSE )
...
ggplot(cv_res, aes(factor(cv_it), cv_error, group = n_runs, color = factor(n_runs))) + geom_point() +  geom_line()  + facet_grid(growth_rate ~ kernel_fun + adaptive) + 
   geom_hline(yintercept = c(null_rms, 0.84310), color = 'red', linetype = 'dashed')
...
LightGBM: 0.84310
```{r submit, echo=FALSE}
  file = file.path(working_folder, "Playground/Feb2021/submit_v4.rbf.csv")
