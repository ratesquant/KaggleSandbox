...
set.seed(132140937)
#top 20 depth = 1, eta = 0.2,subsample = 0.9, min_child_weight = 10

importance_matrix = fread( file.path(working_folder,'santander_transaction/var.imp.csv'))

df_train_set = data.matrix(df[tindex,importance_matrix$Feature[1:20], with = F])

xgb_cv_bayes <- function(eta, max_depth, subsample, min_child_weight) {
  cv <- xgb.cv(params = list(eta = eta,
                             max_depth = max_depth,
                             subsample = subsample, 
                             min_child_weight = min_child_weight,
