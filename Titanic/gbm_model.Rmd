...
model.svm <- svm(svn_formula, data=full_df[train_index,all.vars(svn_formula)], cost = 16, gamma = 0.09)
summary(model.svm)
...
res = ldply(results, .id = 'model', function(x) {
  c(ccr=model_ccr(Survived, x[train_index]),
    ...
    na_count = sum(is.na(x[train_index])))
})
...
fml_exp = as.formula(Survived ~ Pclass + Fare + Title + CabinNumber  + surv_family_count + died_family_count)
...
#fml_cabin = as.formula(Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinNumber + CabinSection)
set.seed(random_seed)
mars_formula_exp = formula (Survived ~ Sex +  Pclass + Siblings + Fare + Title + CabinNumber + surv_family_count + died_family_count)
...
model.rf_exp <- randomForest(Surv ~ Sex +  Pclass + Siblings + Fare + Title + CabinNumber + surv_family_count + died_family_count, 
                             data =  full_df[train_index,],
                             ntree=2000,
                             mtry = 2)
