...
for(vname in c('sub_area', 'full_sq', 'sale_time') ){
  print(plot_profile(log(pred.gbm_cv[train_index]+1), log(df$price_doc[train_index]+1), df[train_index, vname], bucket_count = 10, min_obs = 10, error_band ='normal') + ggtitle(vname))
}
...
```{r cluster_sub_area, fig.width = 8, fig.height = 6, dpi = 150, eval = TRUE, echo=TRUE}
#### cluster sub_area
...
#model.gbm_cv = gbm.more(model.gbm_cv, 3000)
#show best iteration
best_it_res = gbm.perf(model.gbm_res, method = 'test') 
print(best_it_res)
grid()
