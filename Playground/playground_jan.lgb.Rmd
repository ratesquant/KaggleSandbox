...
winsoraze<-function(x, xt, alpha = 0.005) {
...
df[, (w_vars):=lapply(.SD, function(x) winsoraze(x, x[train_index]) ), .SDcols = all_vars]
...
ggplot(df[p_index, ], aes(w_cont3, target)) + geom_point(alpha = 0.5) + geom_smooth(se = FALSE)
...
ggplot(df[train_index & target > 5, ], aes(cont13, cont3, z = target - target_lgb)) + stat_summary_2d(bins = 100)  +  scale_fill_custom(discrete = FALSE)
...
plot_profile(df[train_index, target - target_lgb],  df[train_index, target - target_lgb], 
             df[train_index,  cont13  ], 
             bucket_count = 25, error_band = 'norm') 
...
my_params = data.table(
                       learning_rate = 0.001*seq(7, 9),#0.0110, 
                       bagging_fraction = 0.9830, 
                       min_data = 600, #default 20
                       min_data_in_bin = 7, #default: 3
                       num_leaves = 85)  #default: 31
n_runs = 36
my_params = data.table(
                       ...
                       min_data_in_bin = sample(seq(3, 11),n_runs, TRUE),
...
OPT_Res <- BayesianOptimization(lgb_cv_bayes,
                                bounds = list(
                                learning_rate = c(0.008, 0.02),
                                bagging_fraction = c(0.9, 1.0), #default: 1.0
                                min_data = c(400L, 600L), #default: 20
...
```{r submit, echo=FALSE}
  ...
  file = file.path(working_folder, "Playground/Jan2021/submit_v16.lgb.csv")
