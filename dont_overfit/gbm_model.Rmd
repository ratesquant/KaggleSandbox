...
model.mars <- earth(formula.mars, 
                    ...
                    degree = 2, nfold = 20, trace = 3, glm=list(family=binomial),
...
imp_vars = as.character(unique( subset(coef_path,lambda > cvob3$lambda.min & coef>0)$var_name))
...
#0.04 - 0.847
#0.03 - 0.846
pred.glm    = predict(cvob3,newx=data.matrix(df[,all_vars, with = F]), s=0.035, type = 'response')
pred.glm_lg = predict(cvob3,newx=data.matrix(df[,all_vars, with = F]), s=0.035, type = 'link')
...
save_results( df[,.(id, target = pred.glm)], 'glm')
```
