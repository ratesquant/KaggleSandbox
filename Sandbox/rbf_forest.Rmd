...
timing_res = ldply(seq(nrow(run_cases)), function(i){
 ...
 start_time = Sys.time()
 models = rbf_boot.create(df[,.(x)], df$y, max_nodes = run_cases$max_nodes[i], n_runs = run_cases$boot_samples[i], kernel_fun =rbf_linear_kernel)
...
summary(lm(elapsed  ~ I(max_nodes * max_nodes) + runs, timing_res))
```
...
models_boost =  rbf_boost.create(df[,.(x)],  df$y, max_nodes, boot_runs = 100,  growth_rate =1.5, kernel_fun =rbf_linear_kernel)
...
# CV ---------
cv_res = rbf_boost.create_cv(df[,.(x)],  df$y, max_nodes = 10, boot_runs = 10, max_it = 20, growth_rate =2.0, nfolds = 10, kernel_fun = rbf_linear_kernel, dist_fun = 'L1' )
...
# CV
cv_res = rbf_boost.create_cv(df[,.(x)],  df$y, max_nodes = 100, boot_runs = 30, max_it = 20, growth_rate = 2.0, nfolds = 10, kernel_fun = rbf_linear_kernel, dist_fun = 'L1' )
df_cv_res = data.table(it = seq(nrow(cv_res)), avg = apply(cv_res, 1, mean ), sigma = apply(cv_res, 1, sd ), cv_res )
...
# build model
models_boost =  rbf_boost.create(df[,.(x)],  df$y, boot_runs = 30, max_nodes = 20, max_it = 20, growth_rate =2, kernel_fun = rbf_linear_kernel, dist_fun = 'L1' )
res = rbf_boost.predict(models_boost, df[,.(x)], combine_boots = FALSE)
setDT(res)
