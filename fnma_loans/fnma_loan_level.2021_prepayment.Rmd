...
#df[LOAN_ID == '128633619415', .(date, CURR_RATE, LOAN_AGE,ORIG_UPB, upb, CURRENT_UPB, sbal,unsched_princ, smm, po_ind, Zero_Bal_Code, DLQ_STATUS)]
...
#cum incentive
df[order(date), cum_ei   := 0.01*cumsum(pmax(10, ei)), by =.(LOAN_ID)]
df[order(date), cum_eim  := cumsum(logistic( (ei - 100)/20 )), by =.(LOAN_ID)]
...
lgb_vars = c('ei', 'LOAN_AGE', 'season', 'upb', 'PURPOSE', 'OCC_STAT', 'CHANNEL', 'DTI', 'CSCORE_B', 'CLTV', 'rate_1m48', 'sato') %!in_set% c('id') #best performance
...
model.lgb = lgb.train(params = params_bin, data = dtrain, learning_rate = 0.01,
                      ...
                      eval_freq = 50, boost_from_average = TRUE, verbose = 1)
...
plot_profile(df$po_pred, df$po_ind, df$ei_1m)
plot_profile(df$po_pred, df$po_ind, df$CLTV)
...
plot_profile(df$po_pred, df$po_ind, df$cum_ei)
plot_profile(df$po_pred, df$po_ind, df$cum_eim)
plot_profile(df$po_pred, df$po_ind, df$ei_2m, bucket_count = 50)
plot_profile(df$po_pred, df$po_ind, df$media_ei_lin)
plot_profile(df$po_pred, df$po_ind, df$rate_1m48)
plot_profile(df$po_pred, df$po_ind, df$sato)
...
index   = sample.int(nrow(df), 5e5)
index_v = sample.int(nrow(df), 5e5) %!in_set% index

#Simple cum incentive with LGB
# 10bp is the best threshold  -----------------
res = ldply(seq(0, 50, by = 1), function(x){
  df[order(date), cum_ei_test  := 0.01*cumsum(pmax(x, ei)), by =.(LOAN_ID)]
  
  dtrain.t <- lgb.Dataset(                     data.matrix(df[index,   .(ei, cum_ei_test)]), label = df$po_ind[index],    init_score = logit(df$po_pred[index]))
  dtest.t  <- lgb.Dataset.create.valid(dtrain, data.matrix(df[index_v, .(ei, cum_ei_test)]), label = df$po_ind[index_v],  init_score = logit(df$po_pred[index_v]))

  model.lgb.test = lgb.train(params = list(objective = "binary"), data = dtrain.t, learning_rate = 0.01,
                        valids = list(test = dtest.t),
                        num_threads = 4, nrounds = 1000,  eval_freq = 50, early_stopping_rounds = 100, boost_from_average = TRUE)
...
#Burnout S-Curve  -----------------
burn_params = expand.grid(shift = seq(10, 100, by = 10), width = seq(4, 16, by = 2))
res = ldply(seq(nrow(burn_params)), function(i){
  ...
  dtrain.t <- lgb.Dataset(                       data.matrix(df[index,   c(lgb_vars, 'cum_eim_test'), with = FALSE]), label = df$po_ind[index],    init_score = logit(df$po_pred[index]))
  dtest.t  <- lgb.Dataset.create.valid(dtrain.t, data.matrix(df[index_v, c(lgb_vars, 'cum_eim_test'), with = FALSE]), label = df$po_ind[index_v],  init_score = logit(df$po_pred[index_v]))
  ...
  data.frame(x, it = model.lgb.test$best_iter, error = cv_error[model.lgb.test$best_iter], max_error = max(cv_error), min_error = min(cv_error), shift = burn_params$shift[i], width = burn_params$width[i] )
})
setDT(res)
res[error == min(error)]
ggplot(res, aes(shift, width, fill = error, label = sprintf("%.4f", error) )) + geom_tile() + geom_text() +  scale_fill_custom('mixed', discrete = FALSE)
