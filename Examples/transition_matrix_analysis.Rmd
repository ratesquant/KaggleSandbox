...
rate_sensetivity <- function(df_sim, shift_state_from, shift_state_to, shift_state_bal,  shift = 0.1){
...
rate_sensetivity_full <- function(df_sim, shift = 0.1){
  ...
  df_out = foreach(i = 1:nrow(df_transitions), .combine = 'rbind') %do% {
    df_sim_up = rate_sensetivity(df_sim, df_transitions$state_from[i], df_transitions$state_to[i], 'NA', shift)
...
## balance sensetivity
df_bal_sens = balance_sensetivity (df_sim, 1.0)
...
ggplot(df_bal_sens,aes( shift_state, state_to, bal - bal_base, fill =   bal - bal_base, label = sprintf('%.2f',   bal-bal_base )) ) + geom_tile() + geom_text() +
  scale_fill_gradient2() + ggtitle('Balance sensetivity')
