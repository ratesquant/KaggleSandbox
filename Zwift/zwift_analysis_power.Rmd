...
df[, heart_rate_bkt := cut(heart_rate, breaks = c(0, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0)*hr_max, ordered_result = TRUE, include.lowest = TRUE)]
...
df[order(timestamp),hr_1m_change := as.double(heart_rate -shift(heart_rate, 60)), by = .(file)]
df[order(timestamp),hr_2m_change := as.double(heart_rate -shift(heart_rate, 60 * 2)), by = .(file)]


### monthly summary 
df[, month_date := date -day(date) + 1]

df_monthly = df[, .(time_hours = .N/3600, 
                    hr_blue_pct  = mean(heart_rate < 0.7*hr_max, na.rm= TRUE), 
                    hr_green_pct  = mean(heart_rate < 0.8*hr_max & heart_rate >= 0.7*hr_max, na.rm= TRUE), 
                    hr_yellow_pct = mean(heart_rate < 0.9*hr_max & heart_rate >= 0.8*hr_max, na.rm= TRUE),
                    hr_red_pct    = mean( heart_rate >= 0.9*hr_max, na.rm= TRUE),
                    power_ftp = 0.95*max(power_20m_avg, na.rm=TRUE),
                    power_max_1  = max(power_1m_avg, na.rm=TRUE)), by =.(month_date)]

df_monthly_m =  melt(df_monthly[,.(month_date, hr_green_pct, hr_yellow_pct, hr_red_pct, hr_blue_pct)], id.vars = 'month_date')
ggplot(df_monthly_m, aes(month_date, value, group = variable, color = variable )) + geom_line() + geom_point() + scale_color_manual(values = c('green', 'yellow', 'red', 'blue')) + ggtitle('HR zones')
ggplot(df_monthly_m, aes(month_date, value, group = variable, fill = variable )) + geom_area() + scale_fill_manual(values = c('green', 'yellow', 'red', 'blue')) + ggtitle('HR zones')
ggplot(df_monthly, aes(month_date, time_hours)) + geom_line() + geom_point()
ggplot(df_monthly, aes(month_date, power_ftp)) + geom_line() + geom_point()
ggplot(df_monthly, aes(month_date, power_max_1)) + geom_line() + geom_point()
...
df_agg = df[,.(.N, time=.N/60, kcal = 4.023349 * sum(power, na.rm= TRUE)/4200, date = max(date),
      ...
      hr_green_pct  = mean(heart_rate < 0.8*hr_max & heart_rate > 0.7*hr_max, na.rm= TRUE),
      hr_green_time = sum(heart_rate < 0.8*hr_max & heart_rate > 0.7*hr_max, na.rm= TRUE),
      ...
      hr_1m_drop =  -min(hr_1m_change, na.rm= TRUE),
      hr_2m_drop =  -min(hr_2m_change, na.rm= TRUE),
...
ggplot(df_agg, aes(date, hr_1m_drop, label = sprintf('%.1f', hr_1m_drop ) ))           + geom_point() + geom_smooth(method = 'loess') + geom_text_repel(size = 3)
ggplot(df_agg, aes(date, hr_2m_drop, label = sprintf('%.1f', hr_2m_drop ) ))           + geom_point() + geom_smooth(method = 'loess') + geom_text_repel(size = 3)
ggplot(df_agg, aes(date, hr_green_time, label = sprintf('%.0f', hr_green_time ), size = time, weight = time ))         + geom_point() + geom_smooth(method = 'loess') + geom_text_repel(size = 3)
ggplot(df_agg, aes(date, hr_green_pct, label = sprintf('%.2f', hr_green_pct ), weight = time ))           + geom_point() + geom_smooth(method = 'loess') + geom_text_repel(size = 3)
...
ggplot(dfs) + geom_density(aes(heart_rate), adjust = 0.3) + facet_wrap(~file) + geom_vline(xintercept = c(0.7, 0.8, 0.9)*hr_max, linetype = 'dashed', color = 'grey')
