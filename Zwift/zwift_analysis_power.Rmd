...
df_agg = df[,.(time=.N/60, kcal = 4.023349 * sum(power, na.rm= TRUE)/4200, date = max(date),
...
ggplot(df_agg, aes(date, power_ftp, label = sprintf('%.1f', power_ftp) ))           + geom_point() + geom_smooth(method = 'lm') + geom_text_repel(size = 3)
ggplot(df_agg, aes(date, power_max_1, label = sprintf('%.1f', power_max_1) ))           + geom_point() + geom_smooth(method = 'lm') + geom_text_repel(size = 3)
ggplot(df_agg, aes(date, power_hr_ratio, label = sprintf('%.2f', power_hr_ratio) )) + geom_point() + geom_smooth(method = 'lm') + geom_text_repel(size = 3)
ggplot(df_agg, aes(date, hr_avg, label = sprintf('%.1f', hr_avg) )) + geom_point() + geom_smooth(method = 'lm') + geom_text_repel(size = 3)
ggplot(df_agg, aes(date, cadence, label = sprintf('%.1f', cadence) )) + geom_point() + geom_smooth(method = 'lm') + geom_text_repel(size = 3)
...
last_12_days = head(sort(unique(df$date), TRUE), 25)

dfs = df[date %in% last_12_days]

ggplot(dfs, aes(t/60, factor(date), fill = power_bkt)) + geom_tile() + scale_fill_custom('mixed', discrete = TRUE)
...
ggplot(dfs) + geom_point(aes(power_100s_ewma, heart_rate))  + facet_wrap(~file)
ggplot(dfs) + geom_point(aes(heart_rate, hr_eq)) + geom_abline(slope = 1, linetype = 'dashed', color = 'red') + facet_wrap(~date)
...
ggplot(dfs) + geom_line(aes(t/60, cadence_1m_avg)) + facet_wrap(~file)
...
ggplot(dfs) + geom_point(aes(heart_rate, power), alpha = 0.1) + facet_wrap(~file)
...
df_power_curve = ldply(seq(1, 30*60), function(seconds){
...
ggplot(df_power_curve[last_date == FALSE]) + geom_line(aes(interval, power_max, group = file ), color = 'black', alpha = 0.3) + 
  ...
  geom_vline(xintercept = c(10, 60, 60*10, 60*20, 60*30), linetype = 'dashed', alpha = 0.4)

ggplot(df_power_curve) + geom_line(aes(interval, power_max_last_day_pct, group = file ), color = 'black', alpha = 0.3) +   scale_x_log10() + 
  geom_vline(xintercept = c(10, 60, 60*10, 60*20, 60*30), linetype = 'dashed', alpha = 0.4)
