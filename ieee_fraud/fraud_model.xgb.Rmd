...
ggsave(filename = file.path(working_folder,"ieee_fraud/xgb_vars.pdf"), plot = marrangeGrob(gplots, nrow=2, ncol=2), device = 'pdf', width = 14, height = 8.5, dpi = 240)
...
n_runs = 20
my_params = data.table(depth = sample(seq(from = 5, to = 10),n_runs), 
                       eta = runif(n_runs, 0.001, 0.03), 
                       subsample = runif(n_runs, 0.7, 1.0), 
                       gamma = runif(n_runs, 0, 1.0), 
                       min_child_weight =sample(seq(10),n_runs))
...
ggplot(param_res, aes(depth, test_auc_mean, color = factor(depth), size = min_child_weight )) + geom_point()
