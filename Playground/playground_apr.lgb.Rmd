...
my_params = data.table(ticket_levels = seq(2, 50)) #default = 31

param_res_raw = ldply(seq(nrow(my_params)), function(run_index){
  
  df[, ticket_prefix_ex :=fct_lump_min(ticket_prefix, min = my_params$ticket_levels[run_index])]
...
ggplot(param_res, aes(ticket_levels, best_score)) + geom_point()
