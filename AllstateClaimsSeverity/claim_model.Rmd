...
ggplot(df[train_index1,], aes(x = cont2, log_loss)) + geom_point() + geom_smooth()
ggplot(df[train_index1,], aes(x = cont7, log_loss)) + geom_point() + geom_smooth()
ggplot(df[train_index1,], aes(x = cont14, log_loss)) + geom_point() + geom_smooth()
ggplot(df[train_index1,], aes(x = cont12, log_loss)) + geom_point() + geom_smooth()
...
```{r boxcox, fig.width = 8, fig.height = 6, dpi = 100}

par(mfrow=c(2,1))
boxcox(formula.loss, data = df[train_index2, all.vars(formula.loss)], 
                   lambda = seq(-0.5, 0.5, 1/10))
grid()
boxcox(formula.loss, data = df[train_index3, all.vars(formula.loss)], 
                   lambda = seq(-0.5, 0.5, 1/10))
grid()
...
model.null <- lm(log_loss ~ 1, data = df[train_index2, all.vars(formula.all)])
...
### GBM
gbm (in/out): 0.5428 0.5322 0 (1171.57843)
...
### GBM using training fraction instead of CV
```{r gbm_train, fig.width = 8, fig.height = 6, dpi = 150}
set.seed(random_seed)

max_it = 5000
model.gbm_train = gbm(formula.all, 
                data = df[train_index, all.vars(formula.all)], 
                distribution = 'gaussian',
                n.trees = max_it,
                shrinkage = 0.005,
...
#do extra 100 iterations
#model.gbm_train <- gbm.more(model.gbm_train, 1000, verbose=TRUE)
...
ggplot(res, aes(model, r2_out)) + geom_bar(stat = "identity") + coord_flip() + coord_cartesian(ylim = c(0.9*min(res$r2_out), min(1.0, 1.1*max(res$r2_out) ) ))
