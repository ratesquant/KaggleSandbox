...
gbm_bayes <- function(bag_fraction, my_shrinkage) {
  ...
  return ( list(Score = -min(model.gbm_test$cv.error, na.rm = T),   Pred = pred.gbm_test) )
}

my_bounds = list(bag_fraction = c(0.5, 1.0),
                 my_shrinkage = c(0.1, 0.001))

list(bag_fraction = c(0.1, 1.0),
     my_shrinkage = c(0.1, 0.001),
