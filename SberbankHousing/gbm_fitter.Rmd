...
macro_lag = macro
year(macro_lag$date )<-year(macro_lag$date ) + 1
names(macro_lag) = paste(names(macro_lag), '_lag', sep = '')
df  =  merge(df[,names(df) != 'timestamp'], macro[,names(macro) != 'timestamp'], by.x = 'date', by.y = 'date', all.x = TRUE)
df  =  merge(df[,names(df) != 'timestamp'], macro_lag[,names(macro_lag) != 'timestamp_lag'], by.x = 'date', by.y = 'date_lag', all.x = TRUE)
...
residual_vars =  head(names(df) %!in_set% c(non_vars, all.vars(formula.all)), 18)
...
```{r cluster_sub_area, fig.width = 8, fig.height = 6, dpi = 150, eval = TRUE, echo=FALSE}
#### cluster sub_area
...
residual_vars =  names(df) %!in_set% c('sub_area', non_vars, all.vars(formula.all))
...
plot_gbmpartial(model.gbm_res, best_it_res, "room_sq", output_type = 'link')
