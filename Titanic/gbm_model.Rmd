...
#there are two empty Embarked fields in train set, table(train$Embarked) 
...
train$Siblings = factor(train$Siblings)
train$SiblingsOrdered = factor(train$Siblings,ordered = TRUE)

test$Siblings = as.numeric(as.character(test$SibSp))
test$Siblings[test$Siblings>5] = 5
test$Siblings = factor(test$Siblings)
test$SiblingsOrdered = factor(test$Siblings,ordered = TRUE)

train$Parch = as.numeric(as.character(train$Parch))
train$Parch[train$Parch>5] = 5
train$Parch = factor(train$Parch)
train$ParchOrdered = factor(train$Parch, ordered = TRUE)

test$Parch = as.numeric(as.character(test$Parch))
test$Parch[test$Parch>5] = 5
test$Parch = factor(test$Parch)
test$ParchOrdered = factor(test$Parch, ordered = TRUE)
...
model.gbm.fare = gbm(fare_formula, 
                  data = full_df[!is.na(full_df$Fare), all.vars(fare_formula)], 
...
test$Fare[is.na(test$Fare)]       = predict(model.gbm.fare, newdata = test[is.na(test$Fare),], n.trees =best_it.fare)
full_df$Fare[is.na(full_df$Fare)] = predict(model.gbm.fare, newdata = full_df[is.na(full_df$Fare),], n.trees =best_it.fare)

ggplot(full_df, aes(x = Pclass, Fare)) + geom_boxplot() + geom_jitter()
...
age_formula = as.formula(Age ~ Sex + Pclass + Siblings + Parch + Embarked + Fare)

age_df = full_df[!age_na_index,all.vars(age_formula)]
age_df_estimation = full_df[age_na_index,all.vars(age_formula)]
...
### Linear Model
```{r missing_age_linear, fig.width = 8, fig.height = 4, dpi = 150}
#linear model
age_df_fit = age_df
...
model.lm.age = lm(Age ~  Sex + Pclass + Siblings + Parch + Embarked + fare_spline, 
                  data = age_df_fit)
...
#predictions
fare_splines = bs(full_df$Fare[age_na_index], knots = fare_knots, degree = 1)
fare_splines = fare_splines[,-dim(fare_splines)[2]]
pred_df = full_df[age_na_index,all.vars(age_formula)]
...
#model.npbw.age = npregbw(Age ~  Sex + Pclass + Siblings + Parch + Embarked + Fare, data = age_df)
#model.npbw.age$sfactor$x
bw_scales = c(5.7424297, 0.2465281,2.3573055, 1.3881266, 10.3882008, 49.6081074)

model.npbw.age = npregbw(Age ~  Sex + Pclass + Siblings + Parch + Embarked + Fare, 
          data = age_df, 
...
plot(model.npbw.age, common.scale = FALSE)

model.np.age = npreg(model.npbw.age, newdata = age_df )
age_est_np = predict(model.np.age)
age_pred_np = predict(model.np.age, newdata = age_df_estimation ) 
...
#copy estimated ages to the train_ex data (using GBM method)
train_ex = train 
train_ex$Age[is.na(train_ex$Age)] = predict(model.gbm.age, n.trees = best_it, newdata = train_ex[is.na(train_ex$Age),] ) 

test_ex = test
test_ex$Age[is.na(test_ex$Age)] = predict(model.gbm.age, n.trees = best_it, newdata = test_ex[is.na(test_ex$Age),] ) 
full_df$Age[is.na(full_df$Age)] = predict(model.gbm.age, n.trees = best_it, newdata = full_df[is.na(full_df$Age),] ) 
#ggplot_missing(full_df)
...
test_results.rp = predict(model.rp, newdata = test_ex)
...
test_results.gbm = predict(model.gbm, n.trees = ntrees, newdata = test_ex, type = 'response')
...
test_results.rf = predict(model.rf, newdata = test_ex)
...
test_results.glm = predict(model.glm, newdata = test_ex,  type = 'response')
...
set.seed(123)
model.nnet <- nnet(Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare, data = train_ex, size=6, maxit = 1000, trace=FALSE, decay = 1e-4)
...
test_results.nnet = predict(model.nnet, newdata = test_ex)
...
model.mars <- earth(mars_formula, data = train_ex[, all.vars(mars_formula)], glm=list(family=binomial), degree = 2)
...
test_results.mars = as.vector(predict(model.mars, newdata = test_ex, type = 'response'))
...
#model.npbw <- npcdensbw(Surv ~ Sex + Age + Pclass + SiblingsOrdered + ParchOrdered + Embarked + Fare, data = train_ex) 
...
#bw_scales[3] = 4 * bw_scales[3] # increase scale factor for age to avoid overfitting
...
cat(paste('np:', model_ccr(train_ex$Survived, pred.np )))
plot_binmodel_predictions(train_ex$Survived, pred.np)

test_results.np = as.numeric(as.character( npconmode(model.npbw, newdata = test_ex)$conmode ))
...
results = list()
results$kernel = pred.np
results$logistic = pred.glm
results$gbm = pred.gbm
results$nnet = pred.nnet
results$rf = as.numeric(as.character(pred.rf))
...
res = ldply(results, .id = 'model', function(x) {
  c(ccr=model_ccr(actual, x ),
    ks =model_ks(actual, x ) )
})
...
#0.75598
submit <- data.frame(PassengerId = test$PassengerId, Survived = ifelse( test_results.gbm>=0.5,1,0) )
write.csv(submit, file = "C:/Dev/Kaggle/Titanic/my_solution_gbm.csv", row.names = FALSE)

submit <- data.frame(PassengerId = test$PassengerId, Survived = test_results.np)
write.csv(submit, file = "C:/Dev/Kaggle/Titanic/my_solution_np.csv", row.names = FALSE)

submit <- data.frame(PassengerId = test$PassengerId, Survived =ifelse( test_results.rp>=0.5,1,0))
write.csv(submit, file = "C:/Dev/Kaggle/Titanic/my_solution_rp.csv", row.names = FALSE)

submit <- data.frame(PassengerId = test$PassengerId, Survived = test_results.rf)
write.csv(submit, file = "C:/Dev/Kaggle/Titanic/my_solution_rf.csv", row.names = FALSE)

#0.78947
submit <- data.frame(PassengerId = test$PassengerId, Survived = ifelse( test_results.mars>=0.5,1,0))
write.csv(submit, file = "C:/Dev/Kaggle/Titanic/my_solution_mars.csv", row.names = FALSE)

submit <- data.frame(PassengerId = test$PassengerId, Survived = ifelse( test_results.nnet>=0.5,1,0))
write.csv(submit, file = "C:/Dev/Kaggle/Titanic/my_solution_nnet.csv", row.names = FALSE)

submit <- data.frame(PassengerId = test$PassengerId, Survived = ifelse( test_results.glm>=0.5,1,0))
write.csv(submit, file = "C:/Dev/Kaggle/Titanic/my_solution_glm.csv", row.names = FALSE)
