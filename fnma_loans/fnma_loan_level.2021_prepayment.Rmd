...
refi_scurve * refi_ramp * refi_als * refi_fico * refi_cltv * refi_occ * refi_burn * media_scurve * refi_chn * refi_hpal * refi_hpa * refi_dti * refi_boost * refi_geo * refi_mi * refi_boost2+ 
...
up_lim = c(5,  12,   0, 40,   1, rep(1.0, 5),   rep(1.5, 12),  1,  50, 100, 40,   rep(1.0, 4), 1.8,1.8, -30, -10)
...
df_pd[, model_smm := smm_model(.SD, get_smm_params(init_params))]

ggplot(df_pd, aes(ei, smm2cpr(model_smm) )) + geom_line() + geom_point()  #+ geom_line(aes(ei, model_smm * scurve(ei, 1.0, 1.2, 100, 50)))

#VS date
df_pd = ldply(seq(-200, 300, by = 50), function(ei_in){ 
  df_pd = ldply(sort(unique(df$date)), function(my_date){ 
    a = df[30,]
...
ggplot(df_pd, aes(date, smm2cpr(model_smm), group = ei, color = factor(ei) )) + geom_line() + geom_point()  #+ geom_line(aes(ei, model_smm * scurve(ei, 1.0, 1.2, 100, 50)))
