...
lgb_vars = c('ei', 'LOAN_AGE', 'season', 'upb', 'PURPOSE', 'OCC_STAT', 'CHANNEL','PROP', 'DTI', 'CSCORE_B', 'co_borrower','score_diff', 'FIRST_FLAG', 'CLTV', 'OLTV', 'rate_1m48', 'sato', 'cum_eim', 'geo_ny', 'geo_pr', 'NO_UNITS', 'hpa') %!in_set% c('id') #best performance
...
plots = llply(names(df) %!in_set% c('LOAN_ID', 'CURRENT_UPB', 'Zero_Bal_Code', 'smm', 'Zero_Bal_Code_next'), function(var_name) { #lgb_vars
  print(var_name)
    p = plot_act_vs_model(df, var_name, bucket_count = 10, error_band = 'binom') + ggtitle(var_name) +  theme(title =element_text(size=8))
    return( ggplotGrob(p) )
  })
#marrangeGrob(plots, nrow = 6, ncol = 7, top = NULL)
...
#partial plots --------- 
pdp_index = sample.int(nrow(df), 10000)
df_plot = partialPlot(model.lgb, data.matrix(df[pdp_index,lgb_vars, with = FALSE]), xname = "upb", n.pt = 100)
...
#Burnout S-Curve  -----------------
burn_params = expand.grid(shift = seq(10, 100, by = 10), width = seq(5, 50, by = 5))
...
setDT(res)
res[error == min(error)]
ggplot(res, aes(shift, width, fill = error -  min(error), label = sprintf("%.2e", error -  min(error)) )) + geom_tile() + geom_text() +  scale_fill_custom('mixed', discrete = FALSE)
