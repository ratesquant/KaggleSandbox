...
ggplot(df[state %in% my_states], aes(date, new_cases)) + geom_line() + geom_smooth(span = 0.3, se = FALSE, color = 'red') + facet_wrap(~state, scales = 'free_y')
...
ggplot(df[state %in% inc_states], aes(date, new_cases)) + geom_line() + geom_line(aes(date, new_cases_avg_14d), color = 'red') + facet_wrap(~state, scales = 'free_y') #+ geom_smooth(span = 0.3, se = FALSE, color = 'red', alpha = 0.3)
