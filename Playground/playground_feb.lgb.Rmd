...
ggplot(df[train_index, ], aes(target)) + geom_density(adjust  = 0.1)
...
#LightGBM: Binary
target > 7.5
p(target > 7.5) * E[target |  target > 7.5] + p(target <= 7.5) * E[target |  target <= 7.5]
```{r light_gbm_binary, eval = FALSE}
...
dtrain <- lgb.Dataset(data.matrix(dfs[, lgb_vars , with = FALSE]), label = as.numeric(dfs$target > 7.5), categorical_feature = cat_vars)
...
cv_error = as.numeric(model.lgb_bin$record_evals$valid$binary_logloss$eval_err)
ggplot(data.frame( i = seq(length(cv_error)), cv_error ), aes(i, cv_error)) + geom_line()

min(cv_error)

dm_all = data.matrix(df[,lgb_vars, with = F])
pred.lgb_bin_cvi = ldply(seq(length(model.lgb_bin$boosters)), function(i){ data.frame(cv = i, id = df$id, pred=  predict(model.lgb_bin$boosters[[i]], dm_all)$booster) } )
setDT(pred.lgb_bin_cvi)

pred.lgb_bin_cv_summary = pred.lgb_bin_cvi[, .(.N, avg=mean(pred), sigma = sd(pred)), by =.(cv)]
pred.lgb_bin_cv         = pred.lgb_bin_cvi[, .(.N, avg=mean(pred), sigma = sd(pred)), by =.(id)]

#var_imp   = lgb.importance(model.lgb$boosters[[1]][[1]], percentage = TRUE)
#lgb.plot.importance(var_imp, top_n = 20, measure = "Gain")

df[pred.lgb_bin_cv, target_lgb_bin :=  i.avg, on=.(id)]

lgb_importance = lgb.importance(model.lgb_bin$boosters[[1]][[1]], percentage = TRUE)
lgb.plot.interpretation(lgb_importance)

plot_binmodel_predictions(as.numeric(df$target[train_index]>4.5), df$target_lgb_bin[train_index])
...
#LightGBM: E[target | target > 7.5]
```{r light_gbm_binary2, eval = FALSE}
...
cv_error = as.numeric(model.lgb_2$record_evals$valid$rmse$eval)
ggplot(data.frame( i = seq(length(cv_error)), cv_error ), aes(i, cv_error)) + geom_line()

min(cv_error)

dm_all = data.matrix(df[,lgb_vars, with = F])
pred.lgb_2_cvi = ldply(seq(length(model.lgb_2$boosters)), function(i){ data.frame(cv = i, id = df$id, pred=  predict(model.lgb_2$boosters[[i]], dm_all)$booster) } )
setDT(pred.lgb_2_cvi)

pred.lgb_2_cv_summary = pred.lgb_2_cvi[, .(.N, avg=mean(pred), sigma = sd(pred)), by =.(cv)]
pred.lgb_2_cv         = pred.lgb_2_cvi[, .(.N, avg=mean(pred), sigma = sd(pred)), by =.(id)]

#var_imp   = lgb.importance(model.lgb$boosters[[1]][[1]], percentage = TRUE)
#lgb.plot.importance(var_imp, top_n = 20, measure = "Gain")

df[pred.lgb_2_cv, target_lgb_2 :=  i.avg, on=.(id)]

plot_profile(df[train_index & target > 7.5, target],  df[train_index & target > 7.5, target_lgb_2], 
             df[train_index & target > 7.5,  cont8], bucket_count = 50, error_band = 'norm') 

ggplot(df[train_index & target > 7.5, ], aes(target_lgb_2 - target)) + stat_ecdf()
```


#LightGBM: E[target | target <= 7.5]
```{r light_gbm_binary1, eval = FALSE}
...
cv_error = as.numeric(model.lgb_1$record_evals$valid$rmse$eval)
ggplot(data.frame( i = seq(length(cv_error)), cv_error ), aes(i, cv_error)) + geom_line()

min(cv_error)

dm_all = data.matrix(df[,lgb_vars, with = F])
pred.lgb_1_cvi = ldply(seq(length(model.lgb_2$boosters)), function(i){ data.frame(cv = i, id = df$id, pred=  predict(model.lgb_1$boosters[[i]], dm_all)$booster) } )
setDT(pred.lgb_1_cvi)

pred.lgb_1_cv_summary = pred.lgb_1_cvi[, .(.N, avg=mean(pred), sigma = sd(pred)), by =.(cv)]
pred.lgb_1_cv         = pred.lgb_1_cvi[, .(.N, avg=mean(pred), sigma = sd(pred)), by =.(id)]

#var_imp   = lgb.importance(model.lgb$boosters[[1]][[1]], percentage = TRUE)
#lgb.plot.importance(var_imp, top_n = 20, measure = "Gain")

df[pred.lgb_1_cv, target_lgb_1 :=  i.avg, on=.(id)]

plot_profile(df[train_index & target <= 7.5, target],  df[train_index & target <= 7.5, target_lgb_1], 
             df[train_index & target <= 7.5,  cont8], bucket_count = 50, error_band = 'norm') 

df[, target_lgb_comb := target_lgb_bin  * target_lgb_2  + (1-target_lgb_bin ) * target_lgb_1 ]


ggplot(df[train_index, ], aes(target)) + geom_density(adjust  = 0.1)
ggplot(df[train_index, ], aes(target)) + stat_ecdf()
ggplot(df[train_index, ], aes(target_lgb_comb - target)) + stat_ecdf()

plot_profiles(df$target_lgb_comb[train_index], df[train_index,])

p_index = sample(which(train_index), 10000)
ggplot(df[p_index, ], aes(target_lgb_bin , target)) + geom_point(alpha = 0.2)
ggplot(df[p_index, ], aes(target_lgb_1, target)) + geom_point(alpha = 0.2) + geom_abline(slope = 1, color = 'red')
ggplot(df[p_index, ], aes(target_lgb_2, target)) + geom_point(alpha = 0.2) + geom_abline(slope = 1, color = 'red')

ggplot(df[p_index, ]) + geom_point(aes(target_lgb_2, target), alpha = 0.2) + geom_point(aes(target_lgb_1, target), alpha = 0.2) + geom_abline(slope = 1, color = 'red')
ggplot(df[p_index, ], aes(target_lgb_comb, target)) + geom_point(alpha = 0.2) + geom_abline(slope = 1, color = 'red')
...
v7: 0.84398
v8: 0.84317
v9: 0.84329 - combined p*A + (1-p) * B
```{r submit, echo=FALSE}
  #fwrite(df, file.path(working_folder,'Playground/Feb2021/data/df_lgb.csv'))
 
  file = file.path(working_folder, "Playground/Feb2021/submit_v9.lgb.csv")
  #fwrite(df[test_index, .(id, target=target_lgb)], file = file, row.names = FALSE)
  fwrite(df[test_index, .(id, target=target_lgb_comb)], file = file, row.names = FALSE)
  zip(paste(file, '.zip', sep = ''), file, flags = '-r9Xj')
  print(file)
