...
system.time(model.knn <- train(formula.knn, data = df[tindex, all.vars(formula.knn), with = FALSE], method = "knn",
               trControl = control,
               metric = "ROC",
               tuneGrid = data.frame(k = seq(7,21,2)),
               tuneLength = 10 ))
model.knn
plot(model.knn)

pred.knn = predict(model.knn, df[1:100,], type = 'prob')$x1
