...
library(foreach)
 ...
 #fwrite(df_t[error==0,.(moves, score)], file.path(working_folder,'/connect_x/positions/Test_SOLVED'), sep = ' ', col.names = FALSE)

 #compare with
 df_t1 = fread(file.path(working_folder,'/connect_x/positions/test2eval.log'))
 ...
 a = df_t[error!=0 & name =='Test_L2_R1eval.log']
 a[df_t1, alt_score := i.est_score, on =.(moves)]
 a[alt_score * est_score < 0]
 
 df_t[moves == '2776735234471321']
