...
full_df$CabinNumber = as.factor(50*ceil(as.numeric(cabins_df$CabinNumber)/50))
...
### Recursive Partitioning with Title and Cabin
```{r rpart_cabin, fig.width = 8, fig.height = 6, dpi = 100}
model.rp_cabin = rpart(Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinSection + CabinNumber, 
                 data = full_df[train_index,], 
                 control = rpart.control(cp = 0.005, minsplit = 5))

printcp(model.rp_cabin)

par(mfrow=c(2,1))
plotcp(model.rp_cabin) 
plot(model.rp_cabin, uniform=TRUE) 
text(model.rp_cabin, use.n=TRUE, all=TRUE, cex=.8)

pred.rp_cabin = predict(model.rp_cabin, newdata = full_df)

cat(paste('rpart: ', model_ccr(Survived, pred.rp_cabin[train_index])))
plot_binmodel_predictions(Survived, pred.rp_cabin[train_index])
...
pred.gbm = predict(model.gbm, n.trees = best_it, newdata = full_df, type = 'response')
...
### Random Forest with Title and Cabin
```{r rf_cabin, fig.width = 8, fig.height = 6, dpi = 150}
model.rf_cabin <- randomForest(Surv ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinSection + CabinNumber, data =  full_df[train_index,])

pred.rf_cabin = to_numeric(predict(model.rf_cabin, newdata = full_df))

importance(model.rf_cabin)

cat(paste('random forest:', model_ccr(Survived, pred.rf_cabin[train_index])))
plot_binmodel_predictions(Survived, pred.rf_cabin[train_index])
```
...
### Multivariate Adaptive Regression Splines with Title
```{r mars_title, fig.width = 8, fig.height = 6, dpi = 100}
set.seed(random_seed)
mars_formula = formula (Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title)
model.mars_title <- earth(mars_formula, 
                    data = full_df[train_index, all.vars(mars_formula)], 
                    glm=list(family=binomial), degree = 2, nfold = 3)

plot(model.mars_title)
summary(model.mars_title)
par(mfrow=c(1,1))
plot(evimp(model.mars_title))
#plotmo(model.mars_title)

pred.mars_title = as.vector(predict(model.mars_title, type = 'response', newdata = full_df))

cat(paste('MARS (title):', model_ccr(Survived, pred.mars_title[train_index])))
plot_binmodel_predictions(Survived, pred.mars_title[train_index])
```

### Multivariate Adaptive Regression Splines with Title and Cabin
```{r mars_cabin, fig.width = 8, fig.height = 6, dpi = 100}
set.seed(random_seed)
mars_formula = formula (Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinSection + CabinNumber)
model.mars_cabin <- earth(mars_formula, 
                    data = full_df[train_index, all.vars(mars_formula)], 
                    glm=list(family=binomial), degree = 2, nfold = 3)

plot(model.mars_cabin)
summary(model.mars_cabin)
par(mfrow=c(1,1))
plot(evimp(model.mars_cabin))
#plotmo(model.mars_cabin)

pred.mars_cabin = as.vector(predict(model.mars_cabin, type = 'response', newdata = full_df))

cat(paste('MARS (cabin):', model_ccr(Survived, pred.mars_cabin[train_index])))
plot_binmodel_predictions(Survived, pred.mars_cabin[train_index])
```
...
par(mfrow = c(1, 2), las = 1)
best_it_title = gbm.perf(model.gbm_title, method = 'cv')
grid()
summary(model.gbm_title, n.trees = best_it_title) # influence
grid()

par(mfrow=c(3,3))
for(i in seq(length(all.vars(fml)) - 1)) {
  plot.gbm(model.gbm_title, n.trees =best_it_title,  i = i)
  grid()
}
...
pred.gbm_title = predict(model.gbm_title, n.trees = best_it_title, newdata = full_df, type = 'response')
...
## GBM with Title and Cabin
Title is actualy more important than class. Sex does not matter at this point becuase it is captured by title. Basicaly Title captures both Sex and Class variables.
```{r gbm_cabin, fig.width = 8, fig.height = 6, dpi = 150}
fml_cabin = as.formula(Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinNumber + CabinSection)
#fml = as.formula(Surv ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare)
set.seed(random_seed)

ntrees = 4000
model.gbm_cabin = gbm(fml_cabin, 
                data = full_df[train_index, all.vars(fml_cabin)], 
                distribution = 'bernoulli',
                n.trees = ntrees,
                shrinkage = 0.005,
                bag.fraction = 1.0,
                interaction.depth = 2,
                cv.folds = 3)
print(model.gbm_cabin)

par(mfrow = c(1, 2), las = 1)
best_it_cabin = gbm.perf(model.gbm_cabin, method = 'cv')
grid()
summary(model.gbm_cabin, n.trees = best_it_cabin) # influence
grid()

par(mfrow=c(4,3))
for(i in seq(length(all.vars(fml_cabin)) - 1)) {
  plot.gbm(model.gbm_cabin, n.trees =best_it_cabin,  i = i)
  grid()
}

cat('Two-way interactions') 
gbm_interactions(model.gbm_cabin, full_df[train_index,], 1, 2)

pred.gbm_cabin = predict(model.gbm_cabin, n.trees = best_it_cabin, newdata = full_df, type = 'response')

cat(paste("gbm ccr:", model_ccr(Survived, pred.gbm_cabin[train_index])))
plot_binmodel_predictions(Survived, pred.gbm_cabin[train_index])
```

## GBM with Title, Cabin and Lastname
Title is actualy more important than class. Sex does not matter at this point becuase it is captured by title. Basicaly Title captures both Sex and Class variables.
```{r gbm_name, fig.width = 8, fig.height = 6, dpi = 150}
fml_name = as.formula(Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinNumber + CabinSection + surv_family_count + died_family_count)
#fml = as.formula(Surv ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare)
set.seed(random_seed)

ntrees = 4000
model.gbm_name = gbm(fml_name, 
                ...
                interaction.depth = 2,
                cv.folds = 3)
print(model.gbm_name)

par(mfrow = c(1, 2), las = 1)
best_it_name = gbm.perf(model.gbm_name, method = 'cv')
grid()
summary(model.gbm_name, n.trees = best_it_name) # influence
grid()

par(mfrow=c(4,3))
for(i in seq(length(all.vars(fml_name)) - 1)) {
  plot.gbm(model.gbm_name, n.trees =best_it_name,  i = i)
  grid()
}

cat('Two-way interactions') 
gbm_interactions(model.gbm_name, full_df[train_index,], 1, 2)

pred.gbm_name = predict(model.gbm_name, n.trees = best_it_name, newdata = full_df, type = 'response')

cat(paste("gbm ccr:", model_ccr(Survived, pred.gbm_name[train_index])))
plot_binmodel_predictions(Survived, pred.gbm_name[train_index])
```

## SVM 
```{r svm, fig.width = 8, fig.height = 6, dpi = 150, eval = FALSE}

svn_formula = formula(Surv ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + CabinNumber)
model.svm <- svm(svn_formula, data=full_df[train_index,], cost = 16, gamma = 0.09)
summary(model.svm)
...
# max: 0.791866028708134
cat(paste("svm ccr:", model_ccr(Survived, pred.svm_exp[train_index])))
plot_binmodel_predictions(Survived, pred.svm_exp[train_index])
...
results$gbm_title =pred.gbm_title #0.79904 (0.77990)
results$gbm_cabin =pred.gbm_cabin  #0.78469
results$gbm_name =pred.gbm_name  #
results$glm_s = pred.glm_s
results$nnet = pred.nnet
results$rf = pred.rf
results$rf_cabin = pred.rf_cabin #  0.77990
results$mars = pred.mars #0.76077
results$mars_title = pred.mars_title
results$mars_cabin =pred.mars_cabin
results$rp = pred.rp
results$rp = pred.rp_cabin
...
#fml_cabin = as.formula(Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinNumber + CabinSection)
fml_exp = as.formula(Survived ~ Sex + Age + Pclass + Siblings + Title + CabinNumber + Embarked + family_surv)
set.seed(random_seed)

#0.79904 - my best score
...
#max: 0.779904306220096
cat(paste("gbm ccr:", model_ccr(full_df$Survived[test_index], pred.gbm_exp[test_index])))
plot_binmodel_predictions(full_df$Survived[test_index], pred.gbm_exp[test_index])

#print errors
act_temp = full_df[test_index, all.vars(fml_exp)]
mdl_temp = ifelse(pred.gbm_exp[test_index]>=0.5, 1, 0)
error_index = temp$Survived != mdl_temp
data.frame(temp[error_index,], model = pred.gbm_exp[test_index][error_index])
...
#fml_cabin = as.formula(Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinNumber + CabinSection)
set.seed(random_seed)
mars_formula_exp = formula (Survived ~  Age + Pclass + Siblings + Title + CabinSection + Parch + CabinNumber)
...
#par(mfrow=c(1,1))
#plot(evimp(model.mars_exp))
#plotmo(model.mars_cabin)

pred.mars_exp = as.vector(predict(model.mars_exp, type = 'response', newdata = full_df))

# max:  0.777511961722488
cat(paste("MARS ccr:", model_ccr(full_df$Survived[test_index], pred.mars_exp[test_index])))
...
model.rf_exp <- randomForest(Surv ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + CabinNumber, 
                             data =  full_df[train_index,],
                             ntree=2000,
                             mtry = 2)
...
svn_formula = formula(Surv ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + CabinNumber)
model.svm_exp <- svm(svn_formula, data=full_df[train_index,], cost = 16, gamma = 0.09)
...
set.seed(random_seed)
model.nnet_exp <- nnet(Survived ~ Sex + Age + Pclass + Siblings + Parch + Embarked + Fare + Title + CabinSection + CabinNumber, 
                   data = full_df[train_index,], size=c(6), maxit = 5000, trace=TRUE, decay = 1e-4)
