...
if(file.exists(df_agg_filename)) {
  ...
  df_agg = df[, .(.N, first_date = min(iso_date), last_date = max(iso_date),
                  ...
                  socialEngagementType_n  = count_unique(socialEngagementType),
                  ...
                  date_wday4 = nth_most_freq(date_wday, 4), 
                  ...
                  desktop_n  = sum(device_deviceCategory == 'desktop', na.rm = T),
                  mobile_n  = sum(device_deviceCategory == 'mobile', na.rm = T),
                  tablet_n  = sum(device_deviceCategory == 'tablet', na.rm = T),
                  ...
                  totals_visits    = sum(totals_visits, na.rm = T),
...
df_agg[, first_week_date := weekdays(first_date) ]


df_agg[, total_weeks:= as.numeric(difftime(df_agg$last_date, df_agg$first_date, units = 'weeks'))]
...
all_vars = names(df_agg) %!in_set% c('fullVisitorId','is_train','first_date','last_date','revenue', 'totals_pageviews','totals_hits', 'geo_city','geo_country','geo_region', 'total_days','last_iso_date','first_iso_date', 'last_month', 'first_month')
