...
```{r convert_to_csv}
zwift_fit_files = list.files(file.path(working_folder, 'Zwift/data_new/'), pattern = '*.fit', full.names = TRUE)
...
https://github.com/grimbough/FITfileR
```{r convert_to_csv}
zwift_fit_files = list.files(file.path(working_folder, 'Zwift/hr_comparison/'), pattern = '*.fit', full.names = TRUE)
...
zwift_files = list.files(file.path(working_folder, 'Zwift/hr_comparison/'), pattern = glob2rx('*fit.csv'), full.names = TRUE)
...
ggplot(df) + geom_line(aes(t, power)) + facet_wrap(~file)
ggplot(df) + geom_line(aes(t, heart_rate)) + facet_wrap(~file)
...
gam.model = earth(heart_rate ~ power_avg_16m + power_avg_12m + power_avg_01m + power_avg_15s , df[t>300], nfold = 10, degree = 2, glm=list(family=gaussian))
evimp(gam.model)
plotmo(gam.model)
summary(gam.model)
