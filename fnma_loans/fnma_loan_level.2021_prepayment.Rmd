...
scurve <- function(x, a, b, shift, slope){
  a + (b - a)*logistic((x - shift)/slope)
}
...
smm_model <- function(x, params){
    ...
    total_smm = turn_scurve * turn_ramp * turn_season * turn_fico * turn_cltv * turn_als * turn_occ * turn_hpal * turn_geo + 
      refi_scurve * refi_ramp * refi_als * refi_fico * refi_cltv * refi_occ * refi_burn * media_scurve * refi_chn * refi_hpal * refi_hpa * refi_dti * refi_boost * refi_geo + 
...
params = list('turn_scurve_low' = cpr2smm(5.393896), 'turn_scurve_high' = cpr2smm( 10.04244), 'turn_scurve_shift' = -16.03189, 'turn_scurve_slope' = 29.90699 , #'turn_scurve_slope' = 18.61279, 
              ...
              'turn_geo_x' = c('NY', 'PR', 'NV', 'AZ', 'NJ', 'PA', 'MD'),
              'turn_geo_y' = c(0.65,  0.26, 1.4,  1.4,  0.7,  0.67, 0.67),
              ...
              'refi_scurve_low' = cpr2smm(0), 'refi_scurve_high' = cpr2smm( 20.26784), 'refi_scurve_shift' = 66.79951,	'refi_scurve_slope' = 27.3186, 
              
              'refi_boost_low' = 0.9788007, 'refi_boost_high' = 1.176804, 'refi_boost_shift' =  197.1352,	'refi_boost_slope' = 31.29238,
              ...
              'refi_fico_y'  = c(0.55, 0.7, 0.85,  1.0, 1.2, 1.3, 1.30),
              'refi_fico_na' = 0.8,
              'refi_cltv_x' = c(75,  90), 
              ...
              'media_scurve_low' = 0.7196269, 'media_scurve_high' = 1.669281, 'media_scurve_shift' = -46.93137, 'media_scurve_slope' = -12.89006,
...
plot_act_vs_model2(df, 'hpa', bucket_count = 25)
...
```{r tuning}
refi_obj <- function(refi_high, refi_scurve_shift, refi_scurve_slope) {
  
  params_ex = params
  params_ex['refi_high'] = refi_high
  params_ex['refi_scurve_shift'] = refi_scurve_shift
  params_ex['refi_scurve_slope'] = refi_scurve_slope
...
## Round = 100	refi_high = 0.01836412	refi_scurve_shift = 66.1442	refi_scurve_slope = 25.55715	Value = -15.96842 
OPT_Res <- BayesianOptimization(refi_obj,
                                bounds = list('refi_high' = c(cpr2smm(15), cpr2smm(30)),
...
OPT_Res <- BayesianOptimization(turn_obj,
                                bounds = list('turn_low' = c(cpr2smm(5), cpr2smm(7)),
                                              'turn_high' = c(cpr2smm(8), cpr2smm(11)),
...
#'media_low' = 0.7222222, 'media_high' = 1.633333, 'media_scurve_shift' = -45, 'media_scurve_slope' = -12,
media_obj <- function(media_low, media_high, media_scurve_shift, media_scurve_slope) {

  params_ex = params
  params_ex['media_low'] = media_low
  params_ex['media_high'] = media_high
  params_ex['media_scurve_shift'] = media_scurve_shift
  params_ex['media_scurve_slope'] = media_scurve_slope
...
OPT_Res <- BayesianOptimization(media_obj,
                                bounds = list('media_low'  = c(0.6, 1.0),
                                              'media_high' = c(1.0, 1.7),
                                              'media_scurve_shift' = c(-50, -10),
                                              'media_scurve_slope' = c(-80, -10)),
...
boost_obj <- function(...) {
  input_params <- list(...)
  params_ex = params
  for(param_name in names(input_params)) {
...
OPT_Res <- BayesianOptimization(boost_obj,
                                bounds = list('refi_boost_low'  = c(0.8, 1.0),
...
#-15.90532
OPT_Res <- BayesianOptimization(scurve_obj,
                                bounds = list('refi_boost_low'   = c(0.8, 1.0),
                                              'refi_boost_high'  = c(1.0, 1.2),
                                              'refi_boost_shift' = c(100, 200),
                                              'refi_boost_slope' = c(25, 60),
...
skewed_logistic1 <-function(x, alpha = 1){
  (1+exp(-x))^(-alpha)
}


skewed_logistic2 <-function(x, alpha){
  1-exp(-alpha * x) / (1+exp(-x))^alpha
}
