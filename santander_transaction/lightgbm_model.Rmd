...
```{r plot_data}
  ...
  ggplot(df[1:10000,], aes(var_81, var_139, color = factor(target) )) + geom_point(alpha = 0.2)
  ...
  plots = llply(seq(nrow(all_combinations)), function(i){
    ggplot(df[is_train == TRUE,], aes_string(all_combinations[i,1], all_combinations[i,2])) + geom_bin2d() + facet_wrap(~target) + scale_fill_custom('jet', discrete = FALSE) + theme(legend.position = 'none')
  })
