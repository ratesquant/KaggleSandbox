...
df_sum = df[, .(.N, start_time = min(stime), max_score = max(score), avg_score = mean(score), first_move = sum(is_first == 1), second_move = sum(is_first != 1), 
...
ggplot(df_sum, aes(avg_score, first_pct) ) +  geom_point()
...
ggplot(df[time>0.5], aes(stime, score, group = submission_id)) + geom_line() + facet_wrap(~submission_id) + 
