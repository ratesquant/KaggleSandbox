...
#%% boost runs ----------------
models_boost =  rbf_boost.create(dfs, target, max_nodes = 1281, n_runs = 20, max_it = 20, growth_rate = 2.0, kernel_fun = rbf_mquad_kernel, dist_fun = 'L1' )
...
# CV ---------
cv_res = rbf_boost.create_cv(dfs, target, max_nodes = 3000, n_runs = 10, max_it = 20, nfolds = 5, kernel_fun = rbf_mquad_kernel, dist_fun = 'L1' )
df_cv_res = data.table(it = seq(nrow(cv_res)), avg = apply(cv_res, 1, mean ), sigma = apply(cv_res, 1, sd ), cv_res )
ggplot(df_cv_res[avg>0], aes(it, avg)) + geom_line() + geom_ribbon(aes(it, ymin = avg - sigma, ymax = avg + sigma), alpha = 0.2, fill = 'blue')
