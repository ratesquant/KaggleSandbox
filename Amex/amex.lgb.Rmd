...
lgb_vars = unique(c(all_vars, ex_vars) %!in_set% c('customer_ID', 'S_2'))
...
if(FALSE){
  ...
  ggplot(lgb_importance[Gain > 0.01], aes(fct_reorder(Feature,Gain), Gain)) + geom_bar(stat = 'identity') + coord_flip()
...
```{r submit, echo=FALSE}
  ...
  file = file.path(working_folder, "Amex/data/submit_v06.lgb.csv")
