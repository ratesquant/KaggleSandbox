...
run_cases = expand.grid(nodes = c(10, 60, 100, 150, 200, 300, 400, 500), runs = c(10, 100, 200))

dfs = data.matrix(df[my_index,p_vars, with = FALSE])
target = df$target[my_index]

cv_res = ldply(seq(nrow(run_cases)), function(run_id){
  cv_res = rbf_boot.create_cv(dfs, target, max_nodes = run_cases$nodes[run_id], boot_runs = run_cases$runs[run_id], nfolds = 10,  
                              kernel_params = 1.0, kernel_fun = function(x, c) x, dist_fun = 'L1' )
  data.frame(cv_error = mean(cv_res), cv_sigma = sd(cv_res), max_nodes = run_cases$nodes[run_id], boot_runs = run_cases$runs[run_id])
})

#cv error: 0.8510029
#target: 0.84309
min(cv_res$cv_error) #0.8529316

ggplot(cv_res, aes(max_nodes, cv_error, group = boot_runs, color = factor(boot_runs) )) + geom_point() +  geom_line() + # + geom_smooth(se = FALSE, color = 'black') + 
