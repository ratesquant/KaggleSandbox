...
#ggplot(df, aes(architecturalstyletypeid,logerror)) + geom_point(size = 0.1 ) + geom_smooth()
#ggplot(df, aes(factor(regionidcounty),logerror)) + geom_boxplot()
...
resvars =  unique(names(df) %!in_set% c(non_vars, fun_var))
formula.res = formula (paste( fun_var, ' ~', paste(resvars, collapse = '+')) )
...
plots <- llply(names(df) %in_set% imp_vars, function(vname){
  plot_result = plot_profile(pred.gbm_res, df[,fun_var], df[, vname], bucket_count = 10, min_obs = 10, error_band ='normal') + ggtitle(vname)
  return (plot_result)
})
marrangeGrob(plots, nrow=3, ncol=3)
