...
#'turn_sato_x' =          c(-50, -25, 0, 25, 50), 
params[['turn_sato_y']] =  c(0.9,1.0,1.0, 1.0, 1.1)

df[, model_smm := smm_model(.SD, params)]
objective_function(df, t_index_v) # --6.496013
plot_act_vs_model2(df, 'sato', df$ei < 0, bucket_count = 10)
plot_act_vs_model2(df, 'date', bucket_count = 10)

plot_act_vs_model2(df, 'no_cscore', df$ei < 0, bucket_count = 50)
plot_act_vs_model2(df, 'uer_1m60',df$ei < 0, bucket_count = 10)
plot_act_vs_model2(df, 'uer',df$ei < 0, bucket_count = 10)
plot_act_vs_model2(df, 'CSCORE_B', df$ei < 0, bucket_count = 50)
plot_act_vs_model2(df, 'sato', df$ei < 0, bucket_count = 10)
...
my_vars = unique(c('ei', 'LOAN_AGE', 'season_s',  'upb', 'PURPOSE', 'OCC_STAT', 'CHANNEL','PROP', 'CSCORE_B',  'CLTV', 'OLTV',  'cum_ei','cum_eim', 'media_ei_lin','media_ei_log', 'hpa', 'hpa_life', 
            'uer', 'uer_1m36', 'uer_1m60', 'uer_3m36',  'factor','DTI','no_cscore', 'sato', 'house_value', 'NO_UNITS', 'no_cscore', 'NUM_BO'))
