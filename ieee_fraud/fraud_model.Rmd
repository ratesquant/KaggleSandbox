...
```{r load_data}
 ...
 df[, (fac_vars):=lapply(.SD, function(x) fct_infreq(fct_lump(x, n = 20))), .SDcols = cat_vars]
...
ggsave(filename = file.path(working_folder,"ieee_fraud/gbm.pd.d3.pdf"), plot = marrangeGrob(gplots, nrow=4, ncol=5), device = 'pdf', width = 11, height = 8.5, dpi = 240)

plots = llply(as.character(var_inf$var)[1:35], function(var_name) {
...
vname = 'id_33_ratio'
plot_profile(pred.gbm[s_index], actual[s_index],df[[vname]][s_index], bucket_count = 50, error_band = 'binom')
