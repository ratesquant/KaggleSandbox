...
mon_inc_vars = c('totals_pageviews_sd_log','totals_pageviews_log','totals_pageviews_max_log','count_log','totals_hits_sd')
...
plot_gbmiterations(model.pd) #0.03762, AUC
...
plot_binmodel_roc(actual[tindex], pred.pd[tindex])
plot_binmodel_cdf(actual[tindex], pred.pd[tindex])
plot_binmodel_percentiles(actual[tindex], pred.pd[tindex], 10)
...
plots = llply(all.vars(formula.pd), function(var_name) {
  p = plot_profile(pred.pd[tindex], actual[tindex],df_agg[[var_name]][tindex], error_band = 'binom') +
    ggtitle(var_name) +  theme(title =element_text(size=6))
  return( p )
})
...
#all vars
plots = llply(names(df_agg) %!in_set% exclude_vars, function(var_name) {
  p = plot_profile(pred.pd[tindex], actual[tindex],df_agg[[var_name]][tindex], error_band = 'binom') +
    ggtitle(var_name) +  theme(title =element_text(size=6))
  return( p )
})
...
plot_profile(pred.pd[tindex], actual[tindex],weekdays(df_agg$last_date[tindex]), error_band = 'binom')

plot_profile(pred.pd[tindex], actual[tindex],df_agg$date_wday1[tindex], error_band = 'binom')
...
plots = llply(all.vars(formula.rgs), function(var_name) {
  p = plot_profile(pred.rgs[tindex_rgs], actual[tindex_rgs], df_agg[[var_name]][tindex_rgs], error_band = 'normal') +
    ggtitle(var_name) +  theme(title =element_text(size=6))
  return( p )
})
marrangeGrob(plots, nrow = 4, ncol = 4, top = NULL)
...
plot_profile(pred.rgs[tindex_rgs], actual[tindex_rgs],df_agg$device_browser_major[tindex_rgs], error_band = 'normal')
plot_profile(pred.rgs[tindex_rgs], actual[tindex_rgs],df_agg$device_operatingSystem_major[tindex_rgs], error_band = 'normal')
