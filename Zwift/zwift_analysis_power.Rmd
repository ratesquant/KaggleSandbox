...
bike_ride <- function(t, state, parameters, grade_d, grade_v, power_x, power_v) {
  with(as.list(c(state, parameters)), {
    grade = approx(grade_d, grade_v, X, yleft = 0, yright = 0, rule=2)$y
    alpha = asin(grade)
    power = approx(power_x, power_v, X, yleft = 0, yright = 0, rule=2)$y
    dX <-  V
...
out <- ode(y = state, times = times, func = function(t, state, parameters) bike_ride(t, state, parameters, df_phys$distance, df_phys$grade, df_phys$distance, df_phys$power), parms = parameters)
res = data.table(out)
res[V<0, V:=0 ]
...
df_sim = data.table(distance = seq(0, 4000))
df_sim[, altitude := 10*exp(- (distance - 500) * (distance -500) / 5000)]
df_sim[, altitude := pmax(0, pmin(pmax(0, (distance-1000)/50),  (3000-distance)/50))]
...
ggplot(df_sim, aes(distance, altitude ))     + geom_line() + geom_line(data = res, aes(X, V), color = 'red')
