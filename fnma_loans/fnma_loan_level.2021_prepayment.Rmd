...
add_splines <- function(df, vname, spline_knots, prefix = 'sp_'){
  ...
  spline_names = make.names(stri_join(prefix, vname, '_', spline_knots))
  
  df[, (spline_names) := lapply(seq_len(nk), function(i) x_splined[,i])  ]
  return (TRUE)
}
...
add_splines(df, 'ei', spline_knots = c(-50,  0), prefix = 'sp_turn_')
add_splines(df, 'CSCORE', spline_knots = c(760, 800), prefix = 'sp_turn_')
...
plot_act_vs_model2(df[ei<0], 'CSCORE_B', bucket_count = 20)
...
#fit REFI
turn.offset = predict(model.turn, df, type = 'link')
...
add_splines(df, 'cum_ei',  spline_knots = c( 0, 10),            prefix = 'sp_refi_')
#ggplot(df[sample.int(nrow(df), 1000)], aes(cum_ei, sp_refi_cum_ei_10)) + geom_line()

refi.vars = c(get_vars(names(df), 'sp_refi_'))
refi.formula = as.formula(stri_join('po_ind ~ ', stri_join(refi.vars, collapse = '+')))

#model.refi = glm(refi.formula, data = df[t_index_v], family = binomial(link = "logit"), offset =  turn.offset[t_index_v])
model.refi = glm(refi.formula, data = df, family = binomial(link = "logit"), offset = turn.offset)
summary(model.refi)
