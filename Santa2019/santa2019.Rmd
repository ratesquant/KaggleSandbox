...
```{r load_data}
    ...
    ggplot(df_met[choice == 'choice_0',.(.N, np = sum(n_people)), by =.(day)], aes(day, np)) + geom_point() + 
      geom_line() + geom_hline(yintercept = c(125, 300), linetype = 'dashed', color = 'red') + 
      geom_line(data = df_sol[order(assigned_day),.(.N, np = sum(n_people)), by =.(assigned_day)], aes(assigned_day, np), color ='blue') 
  
  df_sol[,.(.N, np = sum(n_people)), by =.(assigned_day)][np > 300 | np < 125, ]
