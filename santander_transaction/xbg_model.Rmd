...
### Param optimization top 20 features ----------------
...
df_train_set = data.matrix(df[tindex,importance_matrix$var[1:40], with = F])

xgb_cv_bayes <- function(eta, max_depth, subsample, min_child_weight, gamma) {
...
#sink(file.path(working_folder,'santander_transaction/log.top20.txt'), append=FALSE, split=TRUE)
OPT_Res <- BayesianOptimization(xgb_cv_bayes,
                                bounds = list(
                                eta = c(0.01, 0.1), #0.1215
                                max_depth = c(1L, 2L), #1
                                subsample = c(0.7, 1.0), #1.0
                                ...
                                verbose = TRUE)
#sink()
opt_res = data.table(OPT_Res$History, it=t(OPT_Res$Pred))
...
param.formula = as.formula('Value ~ eta + max_depth + subsample + min_child_weight')
...
plots = plot_gbmpartial(param.model, 1000, c('eta', 'max_depth','subsample','min_child_weight'), output_type = 'link')
