...
models_boost =  rbf_boost.create(df[,.(x, y)], df$target, 1, max_nodes = 1024, n_runs = 100, max_it = 100, growth_rate = 2.0, shrink_kernel = 1.0, kernel_fun = rbf_linear_kernel, dist_fun = 'L2', adaptive = FALSE)
...
ggplot(res_all_df[model_id1  > 4 & model_id1 <11,], aes(x, y, fill = y_pred_cum)) + geom_tile() + scale_fill_custom('jet', discrete = FALSE)+ facet_wrap(~model_id1)
