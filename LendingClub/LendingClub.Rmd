...
df[, my_filter := (term_y == '3y' & grade %in% c('A','B','C') & emp_years>=1 & dti<=20 & inq_last_6mths<=1 & mths_since_last_delinq>=24 & annual_inc > 60000)]
...
ggplot(res, aes(issue_d, roi, group = grade, color = grade)) + geom_line()
ggplot(res, aes(issue_d, upb, group = grade, color = grade)) + geom_line()
...
df_s[is.na(loan_dur),loan_dur:=1/12]
df_s[,ret:=total_pymnt/funded_amnt -1.0]
df_s[,roi:=log((total_pymnt+1)/funded_amnt)/loan_dur]
...
allvars = c('grade','emp_years','home_ownership',
            ...
            'pub_rec','pub_rec_bankruptcies','funded_amnt','dti','annual_inc')
...
plots <- llply(c('grade_full','emp_years','home_ownership','verification_status','purpose','sub_grade','initial_list_status','mths_since_last_major_derog','inq_last_6mths','mths_since_last_delinq','pub_rec','funded_amnt','dti','dti_joint') %in_set% names(df), function(vname){
