...
models_boost =  rbf_boost.create(dfs, target, 16, max_nodes = 2080, n_runs = 20, max_it = 12, growth_rate = 1.5, kernel_fun = rbf_mquad_kernel, dist_fun = 'L1', adaptive = FALSE )
...
ggplot(df[train_index], aes(p_cont8, p_cont0, z = target - target_rbf)) + stat_summary_hex(fun = function(x) ifelse(length(x)>10, mean(x), NA), bins = 50) + scale_fill_gradient2()
ggplot(df[train_index], aes(p_cont8, p_cont0, z = target)) + stat_summary_hex(fun = function(x) ifelse(length(x)>10, mean(x), NA), bins = 50)
ggplot(df[train_index], aes(p_cont8, p_cont0, z = target_rbf)) + stat_summary_hex(fun = function(x) ifelse(length(x)>10, mean(x), NA), bins = 50)
...
run_cases = expand.grid(nodes = c(2000), runs = c(20, 30, 50), dist_fun = c('L1'), rbf_kernel = c('rbf_mquad_kernel'), kernel_scale = c(1, 2), growth_rate = c(1.5), shrink_kernel = c(1.0), 
...
#Submit
RBF:      0.84574, 0.84581, 0.84593
LightGBM: 0.84310
```{r submit, echo=FALSE}
  file = file.path(working_folder, "Playground/Feb2021/submit_v7.rbf.csv")
