...
### Logistic PD model 
                                              ...
                                              var    rel.inf
totals_pageviews_sd_log   totals_pageviews_sd_log 34.1640381
totals_pageviews_log         totals_pageviews_log 27.7028449
geo_country_major               geo_country_major 14.3663681
count_log                               count_log 11.2361062
geo_city_major                     geo_city_major  3.9702426
totals_hits_sd_log             totals_hits_sd_log  2.9793656
device_deviceCategory       device_deviceCategory  2.3449133
trafficSource_medium         trafficSource_medium  2.1525235
totals_pageviews_max_log totals_pageviews_max_log  0.8653084
date_wday1                             date_wday1  0.1780877
```{r pd_predict_model}
...
exclude_vars = c('fullVisitorId','is_train','first_date','last_date','revenue','is_revenue', #id and outcome var
                                     'geo_city','geo_country','geo_region','geo_region_major', 'geo_subContinent','geo_region_major', #too many levels
                                     'geo_networkDomain','trafficSource_source','device_browser','device_operatingSystem', #too many levels
                                     'total_days','last_iso_date','first_iso_date', 'last_month', 'first_month',
                                     'totals_pageviews','totals_pageviews_max', 'totals_pageviews_sd', 'totals_hits', 'totals_hits_max', 'N','day_diff_sd',
                                     'date_wday3', 'date_wday2','date_wday1','trafficSource_medium','geo_networkDomain_major','geo_country_major','geo_city_major')

#all_vars = names(df_agg) %!in_set% exclude_vars
all_vars = c('totals_pageviews_sd_log','totals_pageviews_log','count_log','totals_pageviews_max_log','device_deviceCategory','device_browser_major','geo_country_major','geo_city_major','date_wday1','trafficSource_medium', 'totals_hits_sd_log')

set.seed(1012356)

#formula.pd = formula(stri_join( 'is_revenue ~ ', stri_join(all_vars %!in_set% exclude_vars,collapse = ' + ')))
formula.pd = formula(stri_join( 'is_revenue ~ ', stri_join(all_vars, collapse = ' + ')))
...
mon_inc_vars = c('totals_pageviews_log','totals_pageviews_max_log','count_log','totals_pageviews_sd_log')
...
gbm.roc.area(actual.pd[tindex], pred.pd[tindex]) #0.9890211
...
plot_profile(pred.pd[tindex], actual.pd[tindex],weekdays(df_agg$last_date[tindex]), error_band = 'binom')
...
### Revenue Model,  rgs - revenue given sale 
                                                     ...
                                                     var   rel.inf
count_log                                       count_log 27.147888
totals_hits_sd                             totals_hits_sd 23.960792
totals_hits_log                           totals_hits_log 19.412295
device_operatingSystem_major device_operatingSystem_major 12.279053
channelGrouping                           channelGrouping  8.425028
date_wnd_pct                                 date_wnd_pct  4.426837
device_browser_major                 device_browser_major  3.221777
device_deviceCategory               device_deviceCategory  1.126329
...
rgs_vars = c('count_log','totals_hits_sd','totals_hits_log', 'date_wnd_pct','device_operatingSystem_major','device_deviceCategory','channelGrouping', 'device_browser_major')

#formula.rgs = formula(stri_join( 'revenue ~ ', stri_join(all_vars %!in_set% exclude_vars,collapse = ' + ')))
formula.rgs = formula(stri_join( 'revenue ~ ', stri_join(rgs_vars,collapse = ' + ')))

model_vars = all.vars(formula.rgs) %!in_set% c('revenue')
var.monotone = rep(0, length(model_vars))

mon_inc_vars = c('count_log','totals_hits_sd','totals_hits_log')
mon_dec_vars = c('date_wnd_pct')
...
summary(lm('actual ~ model', data.frame(actual = df_agg$revenue[tindex],     model = pred.rev[tindex]))) #0.4273
summary(lm('actual ~ model', data.frame(actual = df_agg$revenue[tindex_rgs], model = pred.rgs[tindex_rgs]))) #0.4273
