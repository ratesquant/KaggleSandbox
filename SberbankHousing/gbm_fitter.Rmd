...
```{r lm_model, fig.width = 8, fig.height = 6, dpi = 150, eval = TRUE, echo=TRUE}
...
## NP model
```{r np_model, fig.width = 8, fig.height = 6, dpi = 150, eval = TRUE, echo=TRUE}
start_time <- proc.time()
...
shrinkage = 0.001, bag.fraction = 0.5,interaction.depth = 2,cv.folds = 5, train.fraction = 1.0,
```{r gbm_cv, fig.width = 8, fig.height = 6, dpi = 150, eval = TRUE, echo=TRUE}
start_time <- proc.time()

non_vars = c('price_log', 'price_doc', 'id', 'timestamp', 'sample', 'sale_year', 'sale_month') #exclude sale_year and month
...
intersect(all.vars(formula.all), insignificant_vars)
