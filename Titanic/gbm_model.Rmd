...
pred.np =  to_numeric( npconmode(model.npbw, newdata = full_df)$conmode )
...
res = ldply(results, .id = 'model', function(x) {
  ggplot(data.frame(actual = Survived, model = x[train_index]), aes(actual, model) )
...
## Parsing Names
```{r parsing_names, fig.width = 8, fig.height = 6, dpi = 100}

full_df$Name[1:10] 
adply(full_df$Name[1:10],1, function(x) {c(title = strsplit(x, split='[,.]')[[1]][2])})
