...
#all_vars = stri_join('var_', c('110', '53', '26', '22', '6', '99', '2', '78'))
...
ggsave(filename = file.path(working_folder,"santander_transaction/gbm.pd.subset.pdf"), plot = marrangeGrob(gplots, nrow=4, ncol=5), device = 'pdf', width = 11, height = 8.5, dpi = 240)

plots = llply(as.character(names(df) %!in_set% c(exclude_vars)), function(var_name) {
...
gplots = lapply(plots, ggplotGrob)
ggsave(filename = file.path(working_folder,"santander_transaction/gbm.profiles.subset.pdf"), plot = marrangeGrob(gplots, nrow=4, ncol=5), device = 'pdf', width = 11, height = 8.5, dpi = 240)
