...
formula.lm = update(formula.all_sig, '~. -cat80')
...
for (cat80_level in levels(df$cat80))
{
  ...
  vars_not_in_train = names(which(sapply(df[,cat_vars], FUN = function(x){
  ...
  vars_to_remove = names(which(sapply(df[seg_index,cat_vars], FUN = function(x) length(unique(x))) <= 1))
...
formula.gbm = update(formula.all_sig, '~. -cat80')
