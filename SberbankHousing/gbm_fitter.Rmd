...
sub_area_cluster_info = subset(sub_area_cluster_info, cluster_algo == 'ward' & cluster_count == 5)
...
#filter out prices which are exactly 1000 (747)
...
train_index = train_index & df$price_doc!=990
...
sig_vars_macro = as.character(read.csv(file.path(working_folder, 'SberbankHousing/sig_vars_macro.csv'), header = FALSE)[,1])
...
#macro plots
macro_melt = melt(macro[, c('date', sig_vars_macro)], id.vars = 'date')
...
allvars =  unique(c(sig_vars, sig_vars_macro) %!in_set% c(non_vars, 'sub_area'))
...
var.monotone[allvars %in% c('full_sq','full_sq_log', 'num_room','state', 'usdrub','eurrub', 'kitch_sq')] =  1
...
imp_vars = as.character(vars.importance_all$var)[vars.importance_all$rel.inf>=.1]
...
allvars =  unique(c(sig_vars, sig_vars_macro, 'cluster_group') %!in_set% c(non_vars, 'sub_area'))
...
var.monotone = rep(0, length(allvars)) #1-increasing, -1 - decreasing, 0: any
var.monotone[allvars %in% c('full_sq','full_sq_log', 'num_room','state', 'usdrub','eurrub', 'kitch_sq', 'mosque_km', 'life_sq', 'cafe_count_5000')] =  1
...
max_it = 80*max_it_mult #64k is for s=0.001, 
...
plots = plot_gbmpartial_2d(model.gbm_cv, best_it_cv, as.character(level2_interactions$vars[level2_interactions$interaction_score>1e-2]), output_type = 'link')
...
```{r cluster_sub_area, fig.width = 8, fig.height = 6, dpi = 150, eval = FALSE, echo=TRUE}
#### cluster sub_area
...
sub_area_res = data.frame(name = df$sub_area[train_index], residual = df$price_log[train_index]- log(pred.gbm_cv[train_index] + 1))
...
files = list.files(path = file.path(working_folder,'SberbankHousing/run1'), pattern = "cluster*", full = TRUE)
...
kable(vars.importance_cv[vars.importance_res$rel.inf>=inf_lowlimit,])

imp_vars = as.character(vars.importance_res$var)[vars.importance_res$rel.inf>=.1]
