...
if(file.exists(df_filename)) {
  ...
  trafficSource_fields = c('campaign', 'source', 'medium', 'keyword',
                           ...
                           'adwordsClickInfo.slot','adwordsClickInfo.gclId','adwordsClickInfo.adNetworkType','adwordsClickInfo.isVideoAd',
                           'referralPath')
  ...
  temp = ldply(sample.int(nrow(df), 10000), function(i) {
     res = data.table(t(unlist(parse_json_all(df$geoNetwork[i]))))  
     return (res)
  } )
  setDT(temp)
  #str(temp)
...
if(file.exists(df_agg_filename)) {
  ...
  #
  #system.time(nth_most_freq(df$channelGrouping))
  #system.time(count_unique(df$channelGrouping))
  ...
  df_agg = df[, .(.N, 
                  ...
                  geo_country_se3 = nth_most_freq(geo_country,3), 
                  ...
                  date_wed_pct = sum(date_wday == 'Wednesday', na.rm = T)/.N, 
                  ...
                  trafficSource_referralPath     = nth_most_freq(trafficSource_referralPath),
                  trafficSource_referralPath_sec = nth_most_freq(trafficSource_referralPath, 2),
                  trafficSource_referralPath_n   = count_unique(trafficSource_referralPath),
                  
                  trafficSource_campaign     = nth_most_freq(trafficSource_campaign),
                  trafficSource_campaign_n   = count_unique(trafficSource_campaign),
                  
                  trafficSource_adContent     = nth_most_freq(trafficSource_adContent),
                  trafficSource_adContent_n   = count_unique(trafficSource_adContent),
                  ...
                  trafficSource_keyword     = nth_most_freq(trafficSource_keyword),
                  trafficSource_keyword_n  = count_unique(trafficSource_keyword),
                  trafficSource_keyword_all = stri_join(unique(trafficSource_keyword), '|'),
                  ...
                  totals_newVisits = sum(totals_newVisits, na.rm = T)/.N,
                  
                  totals_hits_sd      = sd(totals_hits, na.rm = T),
                  totals_pageviews_sd = sd(totals_pageviews, na.rm = T),
                  
                  totals_hits_sk      = skewness(totals_hits, na.rm = T),
                  totals_pageviews_sk = skewness(totals_pageviews, na.rm = T),
                  
                  totals_hits_ku      = kurtosis(totals_hits, na.rm = T),
                  totals_pageviews_ku = kurtosis(totals_pageviews, na.rm = T),
                  
                  totals_hits_max      = max(totals_hits, na.rm = T),
                  totals_pageviews_max = max(totals_pageviews, na.rm = T),
                  
                  totals_hits_min      = min(totals_hits, na.rm = T),
                  totals_pageviews_min = min(totals_pageviews, na.rm = T),
                  
                  totals_hits_q80      = quantile(totals_hits, probs = 0.8, na.rm = T, names = F),
                  totals_pageviews_q80 = quantile(totals_pageviews, probs = 0.8, na.rm = T, names = F),
                  
                  totals_hits_q20      = quantile(totals_hits, probs = 0.2, na.rm = T, names = F),
                  totals_pageviews_q20 = quantile(totals_pageviews, probs = 0.2, na.rm = T, names = F),
...
combine_levels <- function(x, n = 2000){
  counts = table(x)
...
df_agg[,trafficSource_keyword_isset := as.numeric(!is.na(trafficSource_keyword))]
df_agg[trafficSource_keyword %in% c('(not provided)', '(User vertical targeting)','(automatic matching)'), trafficSource_keyword_isset := 0]
...
#limit the size for testing
#df_agg = df_agg[sample.int(nrow(df_agg), 10000),]
...
#only keep several car_11 levels
exclude_vars = c('fullVisitorId','is_train','first_date','last_date','last_iso_date','first_iso_date','revenue', 'is_revenue', 'N','device_deviceCategory_sec') #id and outcome var #replaced with logs

too_many_levels = names(which(sapply(df_agg, function(x) length(levels(x))) >= 100)) #technically max is 1024 

zero_inf_vars.rgs = c("geo_subContinent_n","geo_city_n","device_operatingSystem_sec","trafficSource_source_n", "trafficSource_medium_n",
 "device_browser_sec", "device_browser_n", "total_weeks", 
 "device_operatingSystem_main", "geo_networkDomain_levels",  "geo_subContinent_is_SoutheastAsia",'channelGrouping_main','geo_city_set1','day_diff_sd_log','totals_hits_sd_log','totals_hits_log',
 'date_m1_pct_pos', 'date_m2_pct_pos','date_m3_pct_pos', 'date_m4_pct_pos', 'date_m5_pct_pos','date_m6_pct_pos', 'date_m7_pct_pos','date_m8_pct_pos','date_m9_pct_pos','date_m10_pct_pos','date_m11_pct_pos','date_m12_pct_pos','geo_subContinent_is_EasternAsia','geo_subContinent_is_caribbean','device_operatingSystem_n','geo_networkDomain_is_edu','geo_networkDomain_level3_major','device_browser_main', 'totals_pageviews_sd_log','totals_pageviews_log','totals_bounces_pos','tablet_n','geo_country_se3','geo_continent_n','geo_continent',
 'channelGrouping_n','trafficSource_referralPath_n', 'geo_networkDomain_is_cantv')

all_vars = names(df_agg) %!in_set% c(exclude_vars, too_many_levels, zero_inf_vars.rgs)

set.seed(1012356)

#outcome based sampling - all events and 50 of non-events
#table(df_agg[,.(is_train, is_revenue)])
#table(df_agg$is_train, df_agg$is_revenue, pd_index)
pd_index    = df_agg$is_train
events_n    = sum( df_agg$is_revenue & df_agg$is_train)
nonevents_n = sum(!df_agg$is_revenue & df_agg$is_train)
sample_frac = 2
pd_index[!df_agg$is_revenue & df_agg$is_train][-sample.int(nonevents_n, events_n * sample_frac)] = FALSE
logodds_adjust = log((events_n/nonevents_n)*sample_frac)
...
mon_inc_vars = c('totals_pageviews_max')
mon_dec_vars = c()
...
model.pd  = gbm(formula.pd,
                 ...
                 n.cores = 4,
                 var.monotone = var.monotone,
                 data = df_agg[pd_index , all.vars(formula.pd), with = F],
                 verbose = FALSE)
...
plot_gbmiterations(model.pd)

best_it.pd = gbm.perf(model.pd, plot.it = F) #ifelse(cv_folds==0, max_it, gbm.perf(model.pd, plot.it = F))

pred.pd_link  = predict(model.pd, n.trees = best_it.pd, newdata = df_agg, type = 'link')
pred.pd  = logit(pred.pd_link + logodds_adjust) #mean(pred.pd)

plot_binmodel_predictions(actual.pd[pd_index], pred.pd[pd_index])
plot_binmodel_predictions(actual.pd[tindex], pred.pd[tindex])
gbm.roc.area(actual.pd[tindex], pred.pd[tindex]) #0.9910346
#mean(actual.pd[tindex])
#mean(pred.pd[tindex]) #0.01430119
...
#excluded vars
plots = llply(zero_inf_vars.rgs, function(var_name) {
  #print(var_name)
  p = plot_profile(pred.pd[tindex], actual.pd[tindex],df_agg[[var_name]][tindex], error_band = 'binom') +
    ggtitle(var_name) +  theme(title =element_text(size=6))
  return( p )
...
```{r revenue_predict_model, eval = TRUE}

actual.rgs = df_agg$revenue

tindex_rgs = tindex & df_agg$is_revenue

exclude_vars.rgs = c('fullVisitorId','is_train','first_date','last_date','last_iso_date','first_iso_date','revenue', 'is_revenue','device_deviceCategory_sec', 'N') #id and outcome var #replaced with logs

zero_inf_vars.rgs  = c("geo_continent","geo_continent_n","geo_subContinent_n","geo_country_se3","geo_country_n","geo_city_n","device_operatingSystem_sec","device_operatingSystem_n","device_browser_sec", "device_browser_n", "date_m1_pct_pos", "date_m2_pct_pos", "date_m3_pct_pos", "date_m4_pct_pos", "date_m5_pct_pos", "date_m6_pct_pos", "date_m7_pct_pos", "date_m8_pct_pos", "date_m9_pct_pos", "date_m10_pct_pos", "date_m11_pct_pos", "date_m12_pct_pos", "totals_bounces_pos", "total_weeks", "totals_pageviews_log", "totals_pageviews_sd_log", "totals_hits_log", "totals_hits_sd_log", "day_diff_sd_log","device_operatingSystem_major", "device_operatingSystem_main", "geo_networkDomain_levels", "geo_networkDomain_level3_major",
 "geo_subContinent_is_SoutheastAsia", "geo_subContinent_is_EasternAsia", "geo_subContinent_is_caribbean", "geo_country_is_puertorico")#not picked by GBM - removed to speed-up calc 

too_many_levels = names(which(sapply(df_agg, function(x) length(levels(x))) >= 100)) #technically max is 1024 

all_vars.rgs = names(df_agg) %!in_set% c(exclude_vars.rgs, too_many_levels, zero_inf_vars.rgs)

set.seed(1012356)

#formula.rgs = formula(stri_join( 'revenue ~ ', stri_join(all_vars %!in_set% exclude_vars,collapse = ' + ')))
formula.rgs = formula(stri_join( 'revenue ~ ', stri_join(unique(all_vars.rgs),collapse = ' + ')))

model_vars = all.vars(formula.rgs) %!in_set% c('revenue')
var.monotone = rep(0, length(model_vars))

mon_inc_vars = c('totals_hits_max','count_log', 'total_hits')
mon_dec_vars = c()
...
plot_gbmiterations(model.rgs) # 1.09660

min(model.rgs$cv.error, na.rm = T)#1.096667, 1.096377
...
### Revenue Model (parametric search)
i shrinkage interaction.depth bag.fraction max_it best_it        r2   min_cv
 1:  8      0.01                 9         0.80   2500    1423 0.4491707 1.096604
 2:  7      0.01                 8         0.80   2500    1368 0.4286989 1.096703
 
 i shrinkage interaction.depth bag.fraction max_it best_it        r2   min_cv
1: 5      0.01                12          0.8   1500     994 0.4464169 1.095109
2: 4      0.01                11          0.8   1500    1016 0.4376023 1.095235
3: 3      0.01                10          0.8   1500    1236 0.4468251 1.096002
4: 1      0.01                 8          0.8   1500    1369 0.4293969 1.096335
5: 2      0.01                 9          0.8   1500    1299 0.4384761 1.096377

best params: 0.01, 22, 0.8 
```{r revenue_predict_model_param, eval = FALSE}

params = expand.grid(shrinkage = c(0.01, 0.008, 0.006, 0.005), interaction.depth = c(22), bag.fraction = c(0.8))

print(nrow(params))

param_res = ldply(seq(nrow(params)), function(i) {
  set.seed(1012356)
  ...
  rgs_vars = names(df_agg) %!in_set% c(exclude_vars.rgs, too_many_levels, zero_inf_vars.rgs)
  ...
  model.rgs  = gbm(formula.rgs,
                   ...
                   train.fraction = 1.0,
                   bag.fraction = params$bag.fraction[i],# 0.5 for small samples, 0.7 for large
                   n.cores = 4,
                   var.monotone = var.monotone,
                   data = df_agg[tindex_rgs , all.vars(formula.rgs), with = F],
                   verbose = FALSE)
  
  best_it.rgs = gbm.perf(model.rgs, plot.it = F)
    
  min_cv = min(model.rgs$cv.error, na.rm = T)
  
  pred.rgs = predict(model.rgs, n.trees = best_it.rgs, newdata = df_agg[tindex_rgs,])
  r2 = summary(lm('actual ~ model', data.frame(actual = df_agg$revenue[tindex_rgs], model = pred.rgs)))$r.squared 
  
  return(data.table(i, shrinkage = params$shrinkage[i], interaction.depth = params$interaction.depth[i], bag.fraction = params$bag.fraction[i], max_it = max_it,
                    best_it= best_it.rgs, r2 = r2, min_cv ))
})
#best cv = 1.097665
setDT(param_res)
setorder(param_res, min_cv)

ggplot(param_res, aes(interaction.depth, min_cv, group = shrinkage, color = factor(shrinkage) )) + geom_line() + geom_point() + facet_grid(~bag.fraction)
ggplot(param_res, aes(shrinkage, min_cv, group = interaction.depth, color = factor(interaction.depth) )) + geom_line() + geom_point() + facet_grid(~bag.fraction)

ggplot(param_res, aes(interaction.depth, r2, group = shrinkage, color = factor(shrinkage) )) + geom_line() + geom_point() + facet_grid(~bag.fraction)

ggplot(param_res, aes(interaction.depth, best_it, group = shrinkage, color = factor(shrinkage) )) + geom_line() + geom_point() + facet_grid(~bag.fraction)

```

## Sale probability model (parametric search)
 i shrinkage interaction.depth bag.fraction max_it best_it       roc    min_cv
 1: 23     0.010                 4          0.9   1500    1500 0.9898968 0.2089296
 2: 13     0.010                 4          0.8   1500    1500 0.9899122 0.2089642
 
     i shrinkage interaction.depth bag.fraction max_it best_it       roc    min_cv
 1: 36     0.020                 4         0.85   3000     963 0.9899708 0.2101603
 2:  2     0.010                 4         0.80   3000    1725 0.9899280 0.2103747
 3:  5     0.010                 5         0.80   3000    1412 0.9899709 0.2104726
 4: 37     0.005                 5         0.85   3000    2953 0.9900105 0.2105194
 5: 34     0.005                 4         0.85   3000    2979 0.9898955 0.2105638
 
     i shrinkage interaction.depth bag.fraction max_it best_it       roc    min_cv
 1: 12     0.020                 4         0.80   3000     876 0.9898835 0.2078247
 2: 15     0.020                 5         0.80   3000     880 0.9900108 0.2081137
 3:  9     0.020                 3         0.80   3000    1349 0.9899251 0.2081163
 4: 50     0.010                 3         0.90   3000    2408 0.9899464 0.2081224
 5:
```{r pd_predict_model_params, eval = FALSE}
...
zero_inf_vars.rgs = c("geo_subContinent_n","geo_country_se3","geo_city_n","device_operatingSystem_sec","device_operatingSystem_n","trafficSource_source_n", "trafficSource_medium_n",
 "desktop_n", "device_browser_sec", "device_browser_n", "date_m1_pct_pos", "date_m2_pct_pos", "date_m3_pct_pos", "date_m4_pct_pos", "date_m5_pct_pos",
 "date_m6_pct_pos", "date_m7_pct_pos", "date_m8_pct_pos", "date_m9_pct_pos", "date_m10_pct_pos", "date_m11_pct_pos", "date_m12_pct_pos", "totals_bounces_pos", "total_weeks", "totals_pageviews_log",
 "totals_pageviews_sd_log", "totals_hits_log", "totals_hits_sd_log", "day_diff_sd_log", "device_operatingSystem_main", "geo_networkDomain_levels", 
 "geo_networkDomain_is_edu", "geo_subContinent_is_SoutheastAsia", "geo_subContinent_is_EasternAsia")

all_vars = names(df_agg) %!in_set% c(exclude_vars, too_many_levels, zero_inf_vars.rgs)

params = expand.grid(shrinkage = c(0.005, 0.01, 0.02), interaction.depth = seq(1,7), bag.fraction = c(0.8, 0.85, 0.9))
...
param_res = ldply(seq(nrow(params)), function(i) {
  ...
  events_n    = sum( df_agg$is_revenue & df_agg$is_train)
  nonevents_n = sum(!df_agg$is_revenue & df_agg$is_train)
  pd_index[!df_agg$is_revenue & df_agg$is_train][-sample.int(nonevents_n, events_n)] = FALSE 
  logodds_adjust = log(events_n/nonevents_n)
  ...
  model.pd  = gbm(formula.pd,
    ...
    #plots = plot_gbmpartial(model.pd, best_it.pd, as.character(subset(summary(model.pd, n.trees = best_it.pd, plotit = F),rel.inf>0.1)$var), output_type = 'response')
    #marrangeGrob(plots, nrow = 3, ncol = 4, top = NULL)
    
    pred.pd_link  = predict(model.pd, n.trees = best_it.pd, newdata = df_agg[tindex], type = 'link')
    pred.pd  = logit(pred.pd_link + logodds_adjust)
      
    min_cv = min(model.pd$cv.error, na.rm = T)
    
    roc = gbm.roc.area(actual.pd[tindex], pred.pd[tindex])
    
    #summary(model.pd, n.trees = best_it.pd, plotit = F)
    
    return(data.table(i, 
                      shrinkage = params$shrinkage[i], 
                      interaction.depth = params$interaction.depth[i], 
                      bag.fraction = params$bag.fraction[i], 
                      max_it = max_it,
                      best_it= best_it.pd, roc = roc, min_cv ))
...
ggplot(param_res, aes(interaction.depth, min_cv, group = shrinkage, color = factor(shrinkage) )) + geom_line() + geom_point() + facet_grid(~bag.fraction)
ggplot(param_res, aes(shrinkage, min_cv, group = interaction.depth, color = factor(interaction.depth) )) + geom_line() + geom_point() + facet_grid(~bag.fraction)
ggplot(param_res, aes(interaction.depth, roc, group = shrinkage, color = factor(shrinkage) )) + geom_line() + geom_point() + facet_grid(~bag.fraction)
ggplot(param_res, aes(interaction.depth, best_it, group = shrinkage, color = factor(shrinkage) )) + geom_line() + geom_point() + facet_grid(~bag.fraction)
